...
/**
 * Util class to represent Time. Needed a lightweight class to keep time. The
 * java.util.Clanedar was a bit overkill.
 * 
 * @author mustafa
 * 
 */
public class Time {
    ...
    /**
     * Private constructor to prevent direct instantiation of this object
     * by clients.
     */
    private Time() {
    }
    ...
    /**
     * Builds the Hour & Minute components (Seconds component defaults to 0)
     * from the given value.
     * 
     * @param timeOfDay
     * @return Returns a <code> Time </code> object represented by the timeOfDay
     */
    public static Time buildFromDoubleValue(double timeOfDay) {
        Time returnVal = new Time();
        if (timeOfDay < 0 || timeOfDay > LAST_MINUTE_OF_DAY) {
            throw new IllegalArgumentException("Invalid timeOfDay value supplied. (" + timeOfDay + ")");
        }
        returnVal.hour = (int) Math.floor(timeOfDay);
        if (returnVal.hour == 0) {
            returnVal.minute = (int) (timeOfDay * 60);
        } else {
            returnVal.minute = (int) ((timeOfDay % returnVal.hour) * 60);
        }
        returnVal.seconds = 0;
        return returnVal;
    }
